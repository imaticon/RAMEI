...
{include file="$template/pageheader.tpl" title=$LANG.rcmail_managedomainaliases desc=$domain}
<p>{$LANG.rcmail_managedomaliasedesc}</p>
			...
			<tr>
				...
				<td>
					<form method="post" action="emailmanagement.php?action=managedomainaliases">
						...
						<input type="submit" value="{$LANG.cartremove}" onclick="return confirmDaliasDelete();" class="btn btn-danger"/>
					</form>
				</td>
			</tr>
			{/foreach}			
